...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('verify')
    .setDescription('Manually verify a user (Owner only)')
      ...
      return interaction.editReply({
        embeds: [
          new EmbedBuilder()
            ...
            .setDescription(
              `<@${userId}> has been marked as verified.\n` +
              `**Note:** Manually verified users cannot be auto-added to new servers via \`?pull\`. ` +
              `They will need to join manually or re-verify via OAuth.${roleStatus}`
            )
